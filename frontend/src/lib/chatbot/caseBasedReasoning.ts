...
export class CaseBasedReasoning {
  ...
  private retrieveSimilarCases(currentProblem: Case['problem']): Case[] {
    ...
    // Filter by minimum similarity threshold (0.3 = 30%)
  ...
  public async process(
    ...
    if (similarCases.length === 0) {
      return {
        adaptedResponse: this.buildFallbackResponse(currentProblem),
   ...
   */
  private buildFallbackResponse(problem: Case['problem']): string {
    const emotion = problem.emotion.toLowerCase();
    const intent = problem.intent.toLowerCase();
    
    if (intent.includes('schedule') || intent.includes('plan') 
        || intent.includes('when') || intent.includes('time')) {
      return "What subject and when is your exam? " +
        "I can build your plan from there.";
    }
    
    if (intent.includes('understand') || intent.includes('explain') 
        || intent.includes('help') || intent.includes('stuck')) {
      return "Tell me the specific topic you are " +
        "stuck on and I will give you a concrete approach.";
    }
    
    if (intent.includes('stress') || intent.includes('panic')
        || intent.includes('fail') || intent.includes('scared')
        || intent.includes('screwed')) {
      return "Okay. What is your exam and when is it? " +
        "That is all I need right now.";
    }
    
    if (intent.includes('tired') || intent.includes('motivat') 
        || intent.includes('give up') || intent.includes('cant')) {
      return "Got it. What subject? I will give you " +
        "the smallest possible session that still " +
        "moves the needle.";
    }
    
    return "What are you studying and what do you " +
