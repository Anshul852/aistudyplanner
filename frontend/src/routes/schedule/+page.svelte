<script lang="ts">
  ...
  import { Calendar, Clock, Plus, BookOpen, Brain, RotateCcw, Zap, ChevronLeft, ChevronRight } from 'lucide-svelte';
  ...
  async function fetchSchedule() {
    ...
    if (!currentUser?.session?.access_token) { useFallback(); loading = false; return; }
  ...
  <div class="stats-row">
    {#each [
      ...
      <div class="stat-card" style="--accent:{s.accent}; --glow:{s.glow}; animation-delay:{i*70}ms">
        ...
        <div class="stat-bar"><div class="stat-bar-fill" style="width:{mounted ? Math.min(s.bar*100,100) : 0}%" /></div>
      </div>
    {/each}
  </div>
